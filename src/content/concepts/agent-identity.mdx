...
| `algorithm` | The signing algorithm: `ES256`, `EdDSA`, or `ES256K` |
| `domain` | The domain this identity lives on |

## DID methods

Credat supports three DID methods. Each is suited for different scenarios.
...
### `did:jwk` --- JWK-encoded identity

The full JWK (JSON Web Key) is encoded in the DID.

**When to use:** Interoperability with systems that work with JWKs natively.
