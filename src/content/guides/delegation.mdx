...
```typescript
import { createAgent, generateKeyPair } from "credat";
...
const delegation = await delegate({
  ...
  ownerKeyPair: ownerKeys,             // Owner signs with their private key
...
The `delegation.raw` string is what the agent stores and presents to services. It's a self-contained, signed credential.
...
```typescript
import { hasScope, hasAllScopes } from "credat";
...
```typescript
import { delegate, createStatusList, setRevocationStatus, isRevoked } from "credat";

// 1. Create a status list
const statusList = createStatusList({ size: 1000 });
...
// 3. Later, revoke it
setRevocationStatus(statusList.bitstring, 42, true);

// 4. Check revocation
const revoked = isRevoked(statusList.bitstring, 42);
