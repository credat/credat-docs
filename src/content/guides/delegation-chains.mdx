...
```typescript
import { delegate, generateKeyPair, createAgent } from "credat";
...
```typescript
import { verifyDelegation, resolveDID, jwkToPublicKey } from "credat";
...
const result = await verifyDelegation(delegationB.token, {
  ...
  checkRevocation: async (entry) => {
    const status = await isRevoked(entry.statusListUrl, entry.statusListIndex);
