...
Complete TypeScript type definitions exported by the `credat` package.
...
| Field | Type | Description |
|---|---|---|
| `bitstring` | `Uint8Array` | Compressed bitstring of revocation statuses |
...
| `delete` | `delete(collection, key): Promise<void>` | Delete a value |
| `list` | `list<T>(collection): Promise<T[]>` | List all values in a collection |
