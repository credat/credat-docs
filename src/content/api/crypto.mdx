...
```typescript
import { sign, generateKeyPair } from "credat";
...
```typescript
import { sign, verifySignature, generateKeyPair } from "credat";
...
```typescript
import { publicKeyToJwk, jwkToPublicKey, generateKeyPair } from "credat";
...
Note: `ES256K` (secp256k1) is supported at the agent level via `createAgent` but not exposed in the low-level `generateKeyPair`/`sign`/`verifySignature` functions. Agent creation handles ES256K keys internally.
